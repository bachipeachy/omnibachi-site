#!/usr/bin/env python3
"""Build the pandoc reference template for the Oxford (The Computer Journal) manuscript.

Sibling of make_wiley_reference_docx.py and make_jss_reference_docx.py. Each paper family
owns its own template; none shares state.

*The Computer Journal* asks for single-column, double-spaced text with margins of at least
25 mm, and accepts Word alongside its LaTeX template. Margins here are 1 inch (25.4 mm),
which clears the minimum. Times New Roman is set on the theme and on the default run so
nothing inherits a sans face from pandoc's stock template.

No Lua filter accompanies this family. Double spacing already carries a manuscript well past
the page count of a typeset article, and a page break before every section would add ten more
for no reviewer benefit. Copy `../wiley/wiley_docx_filter.lua` here if a submission wants them.

    python3 make_oxford_reference_docx.py   ->  oxford_reference.docx
"""
import os, re, subprocess, zipfile

SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
SRC = "/tmp/oxford_ref_default.docx"
OUT = os.path.join(SCRIPT_DIR, "oxford_reference.docx")

SERIF      = "Times New Roman"
BODY_PT    = 24   # half-points: 12 pt
TABLE_PT   = 20   # 10 pt — a wide table overruns the page at body size
CAPTION_PT = 20   # 10 pt
AFTER_TW   = 120  # twips: 6 pt after a paragraph
LINE_TW    = 480  # twips: double spacing (240 = single) — OUP asks for double
MARGIN_TW  = 1440 # twips: 1 inch = 25.4 mm, clearing the 25 mm minimum
...
def main():
    ...
    # Headings pull majorHAnsi separately; override every themed face or they come out sans.
    st = re.sub(r'<w:rFonts[^>]*Theme="[^"]*"[^>]*/>', explicit, st)

    # ---- body size, paragraph spacing, double leading ----
    ...
    # A heading or caption stranded at the foot of a page is the most common defect in a
    # generated manuscript; keepNext binds each to what follows.
    for sid in ("Heading1", "Heading2", "Heading3", "Heading4",
                "Caption", "ImageCaption", "TableCaption"):
        st = keep_next(st, sid)

    # pandoc maps `#` to Heading1 (document title), `##` to Heading2, `###` to Heading3.
    # OUP typesets from the accepted source, so this is a reading copy, not the published page.
    ...
    print("%s — %s, body %.0f pt, tables %.0f pt, %s spaced, single column, "
          "%.1f mm margins, page numbers on"
          % (OUT, SERIF, BODY_PT / 2, TABLE_PT / 2,
             "double" if LINE_TW == 480 else "single", MARGIN_TW / 1440 * 25.4))
