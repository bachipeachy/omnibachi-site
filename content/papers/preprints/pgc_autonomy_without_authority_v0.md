...
aliases:
  - /papers/autonomy-without-authority/
...
**Preprint:** [https://doi.org/10.5281/zenodo.22650863](https://doi.org/10.5281/zenodo.22650863) — this page is the frozen rendition of that deposit.

**Submitted to:** Springer Automated Software Engineering — under review.
...
![**Fig. 1** The human role shifted; the authorization point did not. The figure motivates the question and reports no result: the conditions it names are argued, not measured.](/figures/ase/ase_fig1_shifting_role.svg)
...
![**Fig. 2** The whole lifecycle, not the development stage. Rows describe the architecture's intent; Section 8 reports which boundaries were exercised.](/figures/ase/ase_fig2_sdlc_scope.svg)
...
![**Fig. 4** The boundary is set before the agent acts. The containment shown is architectural intent; the annotation on each withheld item names its mechanism, and Section 8.2 reports which were exercised.](/figures/ase/ase_fig4_governing_boundary.svg)
...
![**Fig. 5** Four inputs fixed before the agent acts — three established by a human, one inherited from the previous cycle.](/figures/ase/ase_fig5_what_the_human_supplies.svg)
...
![**Fig. 6** The P0–P8 concern graph. Solid edges are declared priors between adjacent phases; dashed edges are direct citations of P0. Declared priors are a dependency relation, not an enforced input surface.](/figures/ase/ase_fig6_concern_graph.svg)
...
![**Fig. 7** Four concerns across P5–P8, not four names for one specification. Register counts are properties of this implementation at the pinned revision.](/figures/ase/ase_fig7_intent_partition.svg)
...
![**Fig. 8** Three responsibilities, deliberately not fused. Admission and sealing are enforced by the toolchain; promotion is an operator act, with no callable and no actor check in this realization.](/figures/ase/ase_fig8_candidate_to_baseline.svg)
...
![**Fig. 9** Execution realizes; it does not determine. The five prohibitions are separate properties: filled marks the two the reported case stimulated, open marks the three it did not.](/figures/ase/ase_fig9_execution_partition.svg)
...
![**Fig. 10** Direct repository mutation beside governed transformation. The agent performs the engineering work in both; what differs is whether its output is the new state or a candidate.](/figures/ase/ase_fig10_agent_mediated_evolution.svg)
...
![**Fig. 11** Implementation components and artifact flow. Repository counts, published distributions and artifact totals are version-sensitive properties of one pinned revision.](/figures/ase/ase_fig11_component_map.svg)
...
![**Fig. 12** A governed transformation performed by an agent. Solid arrows are observed transitions in this run; the refusal branch is a fixture, not a candidate the worker produced and governance rejected.](/figures/ase/ase_fig12_worked_episode.svg)
...
![**Fig. 13** What the transformation added, and what it left alone. Identifiers are snapshot identities, not projection hashes.](/figures/ase/ase_fig13_baseline_diff.svg)
...
![**Fig. 14** Why a passing demonstration is not evidence that its guard was exercised.](/figures/ase/ase_fig14_mutation_discrimination.svg)
