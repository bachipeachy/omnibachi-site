...
aliases:
  - /papers/architecture-closed-loop-governed-transformation/
publisher: 'Zenodo'
status: 'Deposited'
---
...
This paper takes up the first half of that sentence: how a Protocol Snapshot comes to be declared, validated, and compiled at all. Its companion—*An Architecture for Deterministic Declarative Execution* \[Ganti, 2026l\]—takes up the second half, and treats *nothing runs that was not* as a promise execution itself must keep. Earlier papers in this series established the discipline for construction and execution. They did not fully answer what happens when the Protocol Snapshot itself must change—or who governs the act of changing it. That is the question this paper takes up.

The answer, like its companion's, turns on a single rule about authority:

**Only a human-directed author may create meaning; the machine may derive it, never originate it.**

That rule is not specific to any implementation. It is an architectural model for governed software evolution—and, increasingly, a necessary foundation for software whose implementation may itself be produced and transformed by machines.
...
Every way we have of changing software runs through the same intermediate object: someone writes down
what the system should do — a requirements document, a design, a model, a ticket, a prompt — and
someone or something else turns that into code. The written thing and the running thing then have to
be kept in agreement, forever, by hand. They never quite are. That gap is the source of most of what
we call software maintenance: the spec that no longer matches, the behavior nobody meant, the change
that broke something unrelated.

This paper proposes removing that intermediate object entirely, and it comes down to one rule about
who is allowed to decide what things *mean*.

A change does not start with someone authoring a specification. It starts with a **business problem**
stated in ordinary business language, and with an existing system that already runs — the
**baseline**. From there a governed pipeline carries the problem forward in stages, each stage adding
exactly one layer of meaning and contradicting none of the earlier ones, until what comes out the far
end is not a document but the compiled artifacts of the next running system. Then those artifacts are
validated and **promoted**, and the system that results becomes the baseline the next change starts
from. The loop closes.

The rule that makes this safe is the **Knowledge Partition**: only a human-directed author may create
meaning. Machines — including AI — may derive, transform, check, and compile, but may not decide what
something means. Every derived fact has to say where it came from, so the whole chain from business
problem to running behavior can be walked backwards.

What follows from that rule without further effort:

- **Nobody separately writes a specification,** so no specification can drift out of agreement with
  the system.
- **The system cannot change except by promotion,** so there is one moment of change and it is recorded.
- **Every fact in the running system traces back** to a human decision or to a derivation from one.
- **AI is optional and replaceable.** It is one way to supply human-directed judgment — a worker, not a
  foundation. Swap it out and the guarantees are unchanged, because none of them rest on it.

One clarification, because the claim is easy to misread. The specification does not disappear. Every
job the traditional specification did — stating intent, fixing scope, recording decisions, defining
behavior precisely enough to build from — still gets done. What changes is *where* it is done and *who
authors it*: those jobs are absorbed into the stages of the pipeline as derived, validated,
machine-checkable artifacts, each one traceable to the human decision it came from. The system is
therefore fully specified — more completely and more literally than a prose document could specify it,
since the specification is what compiles and runs. What is removed is the *separately authored,
hand-maintained* specification: the parallel document that had to be written by a person, kept in
agreement with the code by a person, and that always eventually was not. Nobody writes that document
any more; the pipeline produces its content as a by-product of transformation.

The enduring artifact is the surprising part. It is not the code, and it is not a document. It is the
**transformation** itself — the governed process that turns a problem into a baseline. Running systems
become successive products of it. Software stops being a thing you maintain and becomes a thing you
re-derive.

The rest of the paper makes that precise: why the specification was the load-bearing failure (§3–§4),
what replaces it (§5–§6), who may create meaning (§7), why derived facts can be trusted (§8), what
performs the derivation (§9), how a candidate becomes the system (§10), and which claims the evidence
actually supports (§15).
...
Traditional compiler
    precise meaning, lowered in form
    (source → machine code)

Governed transformation
    growing meaning, raised toward execution
    (problem → protocol)
```
...
Stated as a table, the ladder is short enough to hold in view:

| Rung | Newly admitted vocabulary | Still out of bounds |
|---|---|---|
| Business problem | business language only — what is needed and why | any capability, owner, identifier, or ordering |
| Business understanding | what already exists in the baseline, named exactly | anything proposed but not yet existing |
| Business intent | provisional capability names — *what the thing will be* | which part of the system owns it |
| Governance intent | ownership and placement — *whose it is, where it lives* | binding identifiers and wiring |
| Design intent | binding identifiers, artifact shapes, wiring | build order and construction detail |
| Construction | build order — *what to build, in what sequence* | — |

Each row admits exactly one more kind of vocabulary than the row above it, and nothing skips a
row. A stage that uses a term from a lower row has reached above its rung, and that is a
mechanical violation rather than a matter of taste.
...
Figure 2 — The closed loop of governed transformation, in detail.
  ...
  platform:                    │  author:
  deterministic knowledge      │  judgment under authority
  ─────────────────────────────┼───────────────────────────────
                               │
  Baseline Snapshot            │  Business Problem
    presents evidence,  ───────┼─►  (+ declared outcome)
    assigns no relevance       │    disposes evidence,
                               │    decides scope
  ─────────────────────────────┴──────────────┼────────────────
                                              ▼
  enrichment ladder — each stage adds one layer of meaning,
  none contradicts an earlier one; every derived fact carries
  provenance:

     Business Understanding → Business Intent
         → Governance Intent → Design Intent → Construction
             │
             ▼
     ...
     admission compiler   ────►  Admission
             │                   buildable? — the constitutional
             │                   compiler over Baseline ∪ Delta
             ▼
     Implementation       ────►  contracts realized
             │                   (conformance compiler-checked)
             ▼
     Execution Validation ────►  resolves declared outcome?
             │                   ◄── governed gates:
             │                       human approval
             ▼
     Promotion            ────►  installs; verifies integrity;
             │                   judges nothing
             ▼
...
```
Requirements Engineering → CASE → MDD → DSL
    → Low-Code → AI Code Generation → ?
```
...
It is worth naming the objections directly rather than leaving them to be inferred, and saying for
each one what would answer it.

**Designer bias.** The architecture was designed, implemented, and evaluated by the same person. The
judgments that served as ground truth — which artifacts were relevant, which transformation was
better grounded — are that person's. Nothing here rules out the possibility that the model works
because its author knows what it expects. Only a transformation carried out by someone who did not
design the architecture can settle that.

**Single domain.** The transformations reported were run against one reference domain. A domain
chosen by the architect may be one the architecture happens to fit. The claim that progressive
enrichment terminates in admissible artifacts for *any* domain is an architectural argument (§9,
§16), not an empirical result, and should be read as such until a domain the author did not choose
has been transformed.

**No comparative baseline.** No matching change was carried out through a traditional
specification-driven process for comparison, so no claim is made about speed, cost, defect rate, or
effort. The claims here are structural — that certain classes of drift cannot occur — not
quantitative.

**Construct validity of "no specification."** A skeptic may reasonably say the specification has been
renamed rather than removed: the enrichment registers *are* a specification, written in stages. The
answer is that the distinguishing property was never the absence of specifying content but the
absence of a *separately authored, separately maintained* artifact — the derived registers compile,
are validated mechanically, and cannot be edited into disagreement with the system without failing
admission. Whether that distinction holds up under adversarial use is an open question.

**Worker substitutability.** Worker independence was exercised across an automated model worker, an
interactive session, and deterministic replay — three workers, one author, one instruction set. That
the guarantees survive a worker chosen adversarially, or one substantially weaker, is argued from the
Knowledge Partition rather than demonstrated.

**Maturity.** The reference implementation is young, and the reported transformations are few. An
architecture that governs change well over dozens of transformations may still fail to over
thousands; scale effects on the transformation record, on promotion latency, and on the cost of
re-deriving a large baseline are unmeasured.
...
"Governance Dividend" is omitted: it is defined differently across the prior papers, and §14 describes
the phenomenon without it.)*
...
Each stage establishes one thing and is judged before the next may begin. The groupings map onto the
altitudes of §9.

```
Business Problem
  P0  Seed              what the business said, reorganised
  P1  Change Request    the seed restated, each row cited

Business Understanding
  P2  Domain Model      what already exists, verified against the
                        baseline rather than asserted
  P3  Analysis Loop     the decisions: reuse it, extend it, or
                        author it new
...
Governance Intent
  P6  Governance Intent which part of the system owns what

Design Intent
  P7  Design Intent     the artifacts, their identities, wiring

Construction
  P8  Authoring Mandate what to build, and in what order
```
...
design compiler that carries a business problem through its stages, the construction compiler that
turns a validated design into artifacts, the protocol compiler and snapshot assembler that admit them,
and the governed domains the transformations in this paper were run against. Each repository carries
an architecture document written for a reader approaching it without prior familiarity, and a reader
may re-run a transformation rather than take these paragraphs' word for it.
...
[https://doi.org/10.5281/zenodo.21434335](https://doi.org/10.5281/zenodo.21434335)
...
drives this transformation is open source at <https://github.com/protocol-governed-computing>.
