...
aliases:
  - /papers/architecture-deterministic-declarative-execution/
publisher: 'Zenodo'
status: 'Deposited'
---
...
Most of what follows can be read without prior knowledge of Protocol-Governed Computing. The architecture is general. It concerns the relationship between a description of behavior and the machine that carries that behavior out, and that relationship matters whether implementation is written by humans, generated by AI, or produced through fully automated software construction. A reader interested in software architecture, and skeptical of any particular runtime, is exactly the reader this paper is written for.

Still, a few words of orientation will help, because the architecture was worked out and validated in a working reference implementation — open source at <https://github.com/protocol-governed-computing>, and described in the realization companion \[Ganti, 2026m\]. The architecture is offered here on its own terms, but nothing in it is hypothetical: the compiler, the snapshot, the runtime, and the traces this paper argues from are all inspectable.
...
The companion to this paper—*An Architecture for Closed-Loop Governed Transformation*—takes up the first half of that sentence: how a Protocol Snapshot comes to be declared, validated, and compiled. It establishes how governed transformation produces an executable baseline. It treats the second half—*nothing runs that was not*—as a promise that execution itself must preserve.
...
The answer, like its companion's, turns on a single rule about authority:

**The compiler determines behavior; the runtime realizes it.**

That rule is not specific to any implementation. It is an architectural principle for deterministic declarative execution—and, increasingly, a necessary foundation for software whose implementation may itself be produced and transformed by machines.

---

## In plain terms

*Here is the idea before the argument follows.*

Most software keeps its behavior in two places at once. Some of it is written down — in
specifications, diagrams, configuration — and some of it only exists while the program is running, in
the choices the program makes as it goes: a branch taken here, a default filled in there, a retry
nobody wrote down. Because part of the behavior only appears at run time, you cannot fully know what a
system will do by reading anything; you have to run it and watch. That is why production systems
surprise their own authors, why bugs resist reproduction, and why moving software to a new platform
changes how it behaves.

This paper proposes a different arrangement, and it comes down to one rule about who is allowed to
decide.

Before anything runs, a **compiler** takes all the declarations of what the system should do, checks
them, and compiles them into a single sealed artifact called the **executable protocol**. Every
decision is made there, at compile time, and written down. Then a **runtime** picks up that artifact
and carries it out — and is permitted to decide nothing at all. It cannot plan, cannot guess, cannot
fill in a default, cannot fall back. If the protocol does not answer a question, the runtime stops
rather than inventing an answer. We call that rule the **Execution Partition**: *the compiler
determines behavior; the runtime only realizes it.*

The useful part is what follows from that rule without further effort:

- **You can predict what will happen** by reading the protocol, because there is nothing else to read.
- **The same inputs always produce the same run,** because nothing is decided in the moment.
- **A run can be replayed and audited** from the record it leaves behind.
- **The engine is replaceable.** Behavior lives in the protocol, so a different runtime — different
  language, different machine, different decade — running the same protocol does the same thing.

An analogy: a musical score and the orchestra that plays it. The score fixes the music; the orchestra
realizes it. Swap orchestras and it is still the same piece. Conventional runtimes are orchestras that
improvise — sometimes helpfully, always unaccountably. This architecture asks the orchestra not to
improvise, and moves all the composing to where it can be checked before a note is played.

This matters more now than it did, because implementation is increasingly written by machines. When
code can be generated, regenerated, and rewritten faster than anyone can review it, keeping behavior
in the code means losing track of behavior. Keeping it in a compiled, validated protocol means the
generated code can churn freely without the system's behavior drifting with it.

The rest of the paper makes that precise: what the executable protocol must contain (§4), what the
partition forbids (§5), what the runtime does instead of deciding (§6–§7), what determinism and replay
then cost (§8), and which claims the evidence actually supports (§18).
...
The model depends on no particular runtime and on no particular language; the reference
implementation is simply where it was worked out. A worked execution is given in an appendix so
...
describes how a business problem becomes a new executable baseline. This one describes how that
baseline runs: one paper for how software comes to be, one for how it executes. How the architecture
is realized as a functioning platform is the subject of the third \[Ganti, 2026m\].
...
It is, in the phrasing the companion uses, more general than a runtime. A reader may accept the
...
companion asks who may create *meaning*, and answers: only the human-directed author, never the
...
The companion paper closes a loop. It shows how a business problem is transformed, in governed
stages, into a new executable baseline — admitted, validated, promoted. But it treats the moment of
execution as a given. Where it must check that a candidate resolves its problem, it simply says the
candidate is *run* and its behavior observed. It never asks what running requires.

That omission is deliberate, and it leaves a precise question beneath the whole companion. Its
one-sentence summary for newcomers is: *nothing runs that was not first declared, validated, and
compiled into the Protocol Snapshot.* The companion explains *declared, validated, compiled*. It does
...
protocol exactly the drift the companion removes above it?
...
already a consumer of the execution architecture this paper describes. So the companion does not merely
defer execution; it silently depends on it. The architecture below is the machinery the companion's
...
admitted. It is precisely the drift the companion abolishes above the protocol, reappearing below it.
...
The partition is easiest to see in miniature. Consider an ordinary piece of conventional dispatch —
the shape found in almost every service — where the engine decides what to do next:

```
# Conventional: the runtime holds behavioral authority
def run(step, state):
    if state.retries > 0 and step.kind == "remote":
        return retry_with_backoff(step, state)        # a behavior nobody declared
    handler = registry.lookup(step.kind) or default_handler   # a fallback
    plan = planner.plan(step, state)                  # decided now, not before
    return handler(plan, state)
```

Three behaviors are originated here at run time: a retry policy, a default handler, and a plan. None
appears in any artifact that can be read beforehand; all three are recoverable only by running the
system. Under the Execution Partition, each of those decisions has already been made and written
down, and what remains is a walk:

```
# Protocol-governed: the runtime realizes an already-decided structure
def run(node, state):
    contract = node.contract              # resolved at compile time
    outcome  = dispatch(contract, state)  # no lookup, no default
    trace.emit(node, outcome)             # output only
    return node.transitions[outcome]      # routing is data, not logic
```

Nothing was chosen. The retry policy, if there is one, is a declared node in the graph with its own
identity and provenance; the handler was bound by the compiler; the transition is a table entry the
compiler wrote. If `outcome` names no transition, the runtime does not improvise — it halts. The
difference is not that the second version is shorter. It is that everything the second version does
can be known by reading the protocol, and nothing the first version does can be.
...
This last payoff is the one the companion quietly relies on. Its Validation stage checks a candidate's
...
prized. The companion makes *workers* — the sources of judgment that author a transformation —
...
An objection arrives here, and it deserves the direct answer the companion gives its twin. If
...
The architecture can be stated as a small set of invariants, each one a consequence of the Execution
...
- **The runtime originates no behavior.** Every behavior the system exhibits was determined at compile
  time.
- **The executable protocol is immutable.** The runtime reads it and never writes it.
- **The runtime executes only a verified protocol.** Before any traversal, the protocol is confirmed
  to be exactly what the compiler produced and attested, and on mismatch the runtime refuses to run.
- **The execution graph is complete before runtime.** Traversal follows a structure fixed at compile
  time, and no part of it is constructed during the run; this is execution closure.
...
- **Traces are output only.** They are written by the runtime and are never read back as input by any
  component.
- **Contracts define execution interfaces.** The runtime dispatches against declared contracts and
  knows nothing of the implementations beneath them.
- **State topology is protocol-governed.** Stores and their ownership are declared in the protocol,
  not chosen by the runtime.
...
The differences reduce to one question asked of each neighbor — *what, if anything, does its engine
still decide?* — and the answers fit in a single view:

| Approach | Description is declared | What the engine still decides at run time | Behavioral authority |
|---|---|---|---|
| Virtual machines / bytecode | yes (bytecode) | dynamic dispatch, JIT planning, type resolution | shared: compiler + VM |
| Workflow / BPM engines | yes (process graph) | gateway expressions, embedded scripts | shared: modeler + engine |
| Declarative infrastructure | yes (desired state) | the convergence plan — how to reach that state | mostly engine |
| Database query execution | yes (the query) | the execution strategy chosen by the planner | shared: query + planner |
| Rules engines | yes (the rules) | which rules fire, and in what order | mostly engine |
| Functional / declarative languages | yes (the program) | evaluation order, strategy, runtime resolution | shared: program + runtime |
| **Protocol-governed computing** | **yes (executable protocol)** | **nothing — traversal only; halts where the protocol is silent** | **compiler alone** |
...
Read together, the pair makes a single claim in two halves. The companion shows **evolution without
...
plain statement of limits the companion gives its own. The architecture has been realized and
...
validation stage runs on the same machinery. That exercise is not reported on trust: the composition,
its compiler, its runtime, and the traces it produces are open source at
<https://github.com/protocol-governed-computing>, and a reader may rebuild the snapshot and replay a
trace rather than take this paragraph's word for it. There is no second, independently built runtime yet, so
...
The first of these is an open invitation rather than a plan. Runtime independence is the claim that
most needs a second pair of hands: it is argued here, and it will only be *demonstrated* when someone
other than this author builds an interpreter for the same protocol and the traces agree. The material
required is already public — the protocol format, the compiler, the reference runtime, the snapshots,
and the traces to compare against are open source at
<https://github.com/protocol-governed-computing>. Anyone willing to build a second interpreter, to
attack the invariants of §13 and show one of them fails in practice, or to compile a domain of their
own against the architecture, is invited to do so and to open an issue or a discussion in that
repository. Refutation is as welcome as corroboration; both would move the work further than another
paper by the same hand.
...
The reference implementation is open source at <https://github.com/protocol-governed-computing>: the
protocol compiler, the snapshot assembler, the runtime, the inspection surface, and the governed
domains the traces in this paper come from. Each repository carries an architecture document written
for a reader approaching it without prior familiarity.
...
[https://doi.org/10.5281/zenodo.20471804](https://doi.org/10.5281/zenodo.20471804)

Ganti, B. (2026). *Protocol-Governed Systems: Runtime Conceptual Model.* DOI:
[https://doi.org/10.5281/zenodo.20478471](https://doi.org/10.5281/zenodo.20478471)
...
Ganti, B. (2026). *Protocol-Governed Systems: Closed-Loop Governed Evolution* (v1). DOI: [https://doi.org/10.5281/zenodo.21434335](https://doi.org/10.5281/zenodo.21434335)
